	...
	public ResponseEntity<TmbOneServiceResponse<SetCreditLimitResp>> setCreditLimit(
		...
		try {
			...
			if (response.getBody().getStatus().getCode().equals(ResponseCode.SUCESS.getCode())) {
				...
				List<ErrorStatusInfo> errorStatisInfos = response.getBody().getData().getStatus().getErrorStatus();
				creditCardRequestAdjustEvent = creditCardLogService.completeUsageListEvent(creditCardRequestAdjustEvent,
				...
				creditCardRequestAdjustEvent.setReasonForRequest(errorStatisInfos.get(0).getErrorCode());
				...
				return (ResponseEntity<TmbOneServiceResponse<SetCreditLimitResp>>) InternalRespUtil
						.generatedResponseFromService(responseHeaders, oneServiceResponse, errorStatisInfos);
			}
