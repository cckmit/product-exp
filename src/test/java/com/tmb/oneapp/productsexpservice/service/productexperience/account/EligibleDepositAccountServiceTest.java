...
class EligibleDepositAccountServiceTest {
    ...
    private void mockAccountResponse() throws IOException, TMBCommonException {
        ...
        when(productExpAsyncService.fetchCommonConfigByModule(any(), any())).thenReturn(CompletableFuture.completedFuture(commonData));
        when(accountRequestClient.getAccountList(any(), any())).thenReturn(accountList);
    }
